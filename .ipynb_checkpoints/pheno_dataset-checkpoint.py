...
class PhenoDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        #     for k in final_note:
        #         if k == 'overflow_to_sample_mapping':
        #             final_note[k] = torch.LongTensor(final_note[k])
        #         else:
        #             final_note[k] = torch.vstack(
        #                 [torch.LongTensor(final_note[k][i]) for i in selected_chunks])
        ...
        # print(labels.shape)
        if labels.shape[0] > 1:
            print("BEFORRREEE", labels)
            labels = np.sum(labels / np.sum(labels).item(), axis=0)
            print("AFTERRRR", labels)

        return final_note, torch.FloatTensor(labels.squeeze(0))
